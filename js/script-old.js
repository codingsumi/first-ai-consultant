document.addEventListener('DOMContentLoaded', function () {
      ...
      fetch('routes.php', {
        ...
        .then(data => {
          if (data.success) {
            ...
            // messageArea.innerHTML += aiResTpl(data.response);
    ...
    var html = `
                <div class="onetouch-chatbot-mesage user-message" id="user-message">
                    ...
                    <div class="onetouch-chatbot-message-profile">
                      <img src="https://media.istockphoto.com/id/1371301832/photo/excited-young-man-wearing-denim-shirt.jpg?s=612x612&w=0&k=20&c=AtgXnYVh2GpkMGlLwjIYDAwQ6fFvr0ii591bsEaJyLk="/>
                    </div>
                </div>`;
  ...
  function displayAiResponse(message) {
    ...
    if (typeof message !== 'string') {
      ...
      message = JSON.stringify(message);  // Convert non-string to string for display
    }
    ...
    var aiMessageTpl = `
                <div class="onetouch-chatbot-mesage bot-message">
                    ...
                    <div class="onetouch-chatbot-message-box">
                    <div class="onetouch-innercontent-box" id="${messageId}"></div>
                      <span class="time"
                        >${formattedDate}
                        <span>
                        <svg
    ...
    messageArea.innerHTML += aiMessageTpl;

    const aiMessageText = document.getElementById(messageId);
    
    typeWriter(aiMessageText, cleanResponse(message));
    messageArea.scrollTo({
      top: messageArea.scrollHeight,
      behavior: 'smooth',
    });
  ...
  function typeWriter(element, text, delay = 20) {
    ...
    function type() {
      if (index < text.length) {
        ...
        index++;
        setTimeout(type, delay); // Recursive call for the next character
      }
    }
